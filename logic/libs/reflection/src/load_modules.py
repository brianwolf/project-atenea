...
def file_name(path: str) -> str:
    ...
    """
    return os.path.project-ateneaname(path).split('.')[0]


def get_modules_paths(project-atenea_path: str) -> List[str]:
    """
    ...
    for root, _, files in os.walk(project-atenea_path):
...
def get_modules_paths_by_regex(regex_path: str) -> List[str]:
    ...
    for project-atenea_path in glob.glob(regex_path):
        paths.extend(get_modules_paths(project-atenea_path))
